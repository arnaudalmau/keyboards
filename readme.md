...
The only files modified from the default branch are [config.h](https://github.com/arnaudalmau/keyboards/blob/main/Snap%20-%20QMK/config.h), [rules.mk](https://github.com/arnaudalmau/keyboards/blob/main/Snap%20-%20QMK%20/rules.mk) and [keymap.c](https://github.com/arnaudalmau/keyboards/blob/main/Snap%20-%20QMK/keymap.c) to accommdate the double encoder, double OLED with bongo cat animation on both sides and my custom layouts from above. You can safely git clone the original qmk repository and replace these three files to have it working. All is under the [/Snap - QMK](https://github.com/arnaudalmau/keyboards/tree/main/Snap%20-%20QMK%20) folder. I used a ProMicro that doesn't need flash _per se_ but uses .uf2 files dragged and dropped directly to it. My precompiled .uf2 file can be found [here](https://github.com/arnaudalmau/keyboards/blob/main/Snap%20-%20QMK/nullbitsco_snap_arnaudalmauv3_rp2040_ce.uf2). If you're adventurous enough, you could even fit a Nice!Nano and make it wireless, but expect little battery life if you're using the standard ~110mA battery (roughly an hour with underglow on, around 30-35 hours without it).
### Pros
- The ultimate DIY adventure!
- Almost endlessly customizable (if you're into 75% layout).
- Double OLED for double fun!
- Splittable... or not. Your choice.

### Cons
- Good soldering skills required.
- Can get quickly expensive if you start adding extras (millmax mods specially).
- You have to get creative if you like tilting your boards.
