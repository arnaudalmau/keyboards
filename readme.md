...
The only files modified from the default branch are [config.h](https://github.com/arnaudalmau/keyboards/blob/main/Snap%20-%20QMK%20/config.h), [rules.mk](https://github.com/arnaudalmau/keyboards/blob/main/Snap%20-%20QMK%20/rules.mk) and [keymap.c](https://github.com/arnaudalmau/keyboards/blob/main/Snap%20-%20QMK%20/keymap.c) to accommdate the double encoder, double OLED with bongo cat animation on both sides and my custom layouts from above. You can safely git clone the original qmk repository and replace these three files to have it working. All is under the [/Snap - QMK](https://github.com/arnaudalmau/keyboards/tree/main/Snap%20-%20QMK%20) folder. I used a ProMicro that doesn't need flash _per se_ but uses .uf2 files dragged and dropped directly to it. My precompiled .uf2 file can be found [here](). If you're adventurous enough, you could even fit a Nice!Nano and make it wireless, but expect little battery life if you're using the standard ~110mA battery (roughly an hour with underglow on, around 30-35 hours without it).
### Pros
